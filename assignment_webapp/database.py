...
def user_playlists(username):
    ...
    try:
        ...
        ###############################################################################
        sql = """select collection_id,collection_name,count(MediaItem)
                from mediaserver.MediaCollection inner join mediaserver.MediaCollectionContents using(collection_id)
                inner join mediaserver.MediaItem using(media_id)
                where username = %s  
...
def user_podcast_subscriptions(username):
    ...
    try:
        ...
        sql = """select podcast_id,podcast_title,podcast_uri,podcast_last_updated
                from mediaserver.Podcast inner join mediaserver.Subscribed_Podcasts using(podcast_id)
                where username = %s
        """
...
def get_podcastep(media_id):
    ...
    try:
        ...
        sql = """
        SELECT *
        FROM mediaserver.podcastepisode INNER join mediaserver.metadata ON (media_id = md_id)
        WHERE media_id = %s
        """
...
def get_podcast_metadata(podcast_id):
    ...
    try:
        ...
        # If there were any errors, return a NULL row printing an error to the debug
        print("Unexpected error getting podcast metadata for ID: "+podcast_id, sys.exc_info()[0])
        raise
    cur.close()                     # Close the cursor
    conn.close()                    # Close the connection to the db
    return None
...
def find_matchingalbums(searchterm):
    ...
    try:
        ...
        # If there were any errors, return a NULL row printing an error to the debug
        print("Unexpected error getting albums:", sys.exc_info()[0])
        raise
    cur.close()                     # Close the cursor
    conn.close()                    # Close the connection to the db
    return None
...
def get_album_genres(album_id):
    ...
    try:
        ...
        from ((((((mediaserver.Album_songs ams
        inner join mediaserver.song s using (song_id))
        inner join mediaserver.audiomedia am on(am.media_id = s.song_id))
        inner join mediaserver.mediaitem mi using(media_id))
        inner join mediaserver.mediaitemmetadata mimd using(media_id))
        inner join mediaserver.metadata md using(md_id))
        inner join mediaserver.metadatatype mdt using(md_type_id))
        where lower(mdt.md_type_name) = 'song genre' and ams.album_id = %s ;
        """
...
def get_suggested_movies():
    """
    Random suggest some movies for user.
    """
