...
def single_podcast(podcast_id):
    ...
    podcastmetadata = database.get_podcast_metadata(podcast_id)

    podcastepisodes = None
    podcastepisodes = database.get_all_podcasteps_for_podcast(podcast_id)

    # Data integrity checks
    if podcast == None:
        podcast = []

    if podcastmetadata == None:
        podcastmetadata = []

    if podcastepisodes == None:
        podcastepisodes = []
...
def get_suggested_movies():
    ...
    page['title'] = 'Suggested movies'

    # Get a list of all suggested movies from the database
    movies = None
    movies = database.get_suggested_movies()
    ...
    return render_template('listitems/suggestedmovie.html',
                           session=session,
                           page=page,
                           user=user_details,
                           movies=movies)
...
def search_movies():
    ...
    if request.method == 'POST':
        ...
        movie = database.find_matchingmovies(request.form['searchterm'])
    ...
    if movie == None or movie == []:
        ...
        page['bar'] = True
        flash('Found '+str(len(movie))+' results!')
        session['logged_in'] = True
...
def add_song():
    ...
    if request.method == 'POST':
        ...
        song = database.add_song_to_db(newdict['storage_location'],newdict['description'],newdict['song_title'],newdict['song_length'], newdict['song_genre'], newdict['artist_id'])
...
def search_albums():
    ...
    page['title'] = 'Albums Search' # Add the title
    ...
    if albums == None or albums == []:
        ...
        # NOTE :: YOU WILL NEED TO MODIFY THIS TO PASS THE APPROPRIATE VARIABLES
    return render_template('searchitems/search_album.html',
                           session=session,
                           page=page,
                           user=user_details,
                           albums=albums )